...
# ── Vivado ────────────────────────────────────────────────────────────────────
# Full path to vivado.bat used for the main flow (build bitstream, program,
# sim, etc.) — stays on the regular toolchain version.
$env:VIVADO          = "C:\AMDDesignTools\2026.1\Vivado\bin\vivado.bat"

# Full path to vivado.bat used ONLY for 1.GenerateAxiIp.ps1.
# Pinned to 2022.2 to test the create_peripheral AXI4-Lite IP generated by
# 2022.2, which (per docs/known-bug-discussion.md) is reported to be AMBA
# AXI4-Lite protocol compliant, unlike the 2024.2+ generated code.
$env:VIVADO_IP_GEN   = "C:\Xilinx\Vivado\2022.2\bin\vivado.bat"
...
Write-Host "  VIVADO_IP_GEN   : $env:VIVADO_IP_GEN"
...
if (-not (Test-Path $env:VIVADO_IP_GEN)) {
    Write-Warning "VIVADO_IP_GEN not found at $env:VIVADO_IP_GEN -- update the path above."
} else {
    Write-Host "  Vivado (IP gen) OK" -ForegroundColor Green
}